...
# Helper function to render a PDF page to PIL Image
def render_pdf_page(doc, page_num, dpi=180):
    page = doc[page_num - 1] # 1-based index to 0-based
    zoom = dpi / 72.0
...
# Helper function to crop top half or bottom half of a page
def crop_page_region(img, region="full"):
    w, h = img.size
    # Crop out outer margins if needed, keeping inner content
    if region == "top":
        return img.crop((0, 0, w, int(h * 0.52)))
    elif region == "bottom":
        return img.crop((0, int(h * 0.48), w, h))
    elif region == "middle":
        return img.crop((0, int(h * 0.25), w, int(h * 0.75)))
    return img

# Helper function to stitch multiple PIL images vertically
def stitch_images(img_list):
    ...
    # Calculate combined height and max width
...
def process_subject(doc, mapping, out_dir, prefix):
    for q_id, parts in mapping.items():
        img_parts = []
        for page_num, region in parts:
            if page_num > len(doc):
                continue
            raw_img = render_pdf_page(doc, page_num)
            cropped_img = crop_page_region(raw_img, region)
        ...
        if stitched:
            ...
            print(f"Saved: {prefix}_{q_id}.png ({stitched.width}x{stitched.height})")

print("\n--- Processing Matematik (50 Questions) ---")
process_subject(doc_mat, mat_mapping, out_mat_dir, "mat")

print("\n--- Processing Türkçe (50 Questions) ---")
process_subject(doc_tr, tr_mapping, out_tr_dir, "tr")

print("\nAll 100 question images extracted and stitched successfully!")
