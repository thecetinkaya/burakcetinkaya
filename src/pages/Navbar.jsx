...
const Navbar = () => {
  ...
  const getLinkClass = (path) => {
    ...
    return `navbar-link font-extrabold text-sm transition-all cursor-pointer ${isActive ? "text-[#13d179]" : "text-slate-500 dark:text-slate-400 hover:text-[#13d179]"
      }`;
  };

  const getMobileLinkClass = (path) => {
    const isActive = location.pathname === path;
    return `navbar-link font-black text-base transition-all cursor-pointer ${isActive ? "text-[#13d179]" : "text-slate-600 dark:text-slate-300 hover:text-[#13d179]"
      }`;
  };
    ...
    <nav className="navbar fixed top-0 left-0 w-full z-50 bg-white/95 dark:bg-[#0b0f19]/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-800/60 shadow-lg transition-colors duration-300">
      ...
      <AnimatePresence>
        {isMenuOpen && (
          ...
          >
            <Link to="/" className={getMobileLinkClass("/")} onClick={closeMenu}>Home</Link>
            <Link to="/about" className={getMobileLinkClass("/about")} onClick={closeMenu}>About</Link>
            <Link to="/projects" className={getMobileLinkClass("/projects")} onClick={closeMenu}>Projects</Link>
            <Link to="/contact" className={getMobileLinkClass("/contact")} onClick={closeMenu}>Contact</Link>

            <button
              onClick={() => { toggleTheme(); closeMenu(); }}
              className="flex items-center gap-2 font-black text-base text-slate-600 dark:text-slate-300 hover:text-[#13d179] dark:hover:text-[#13d179] transition py-1 cursor-pointer"
            >
              {theme === "dark" ? (
                <> <FaSun size={15} className="text-amber-400" /> <span>Açık Tema</span> </>
              ) : (
