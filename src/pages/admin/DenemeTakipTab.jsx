...
const DenemeTakipTab = ({ theme }) => {
  ...
  const [books, setBooks] = useState(() => {
    ...
    if (savedV3) {
      try { return JSON.parse(savedV3); } catch (e) {}
    }
  ...
  useEffect(() => {
    localStorage.setItem("kpss_deneme_kaynaklari_v3", JSON.stringify(books));
  }, [books]);
  ...
  const handleSaveDeneme = (e) => {
    ...
    const correct = Math.max(0, parseInt(formData.correct) || 0);
    const incorrect = Math.max(0, parseInt(formData.incorrect) || 0);
    const empty = Math.max(0, parseInt(formData.empty) || 0);
    const net = parseFloat((correct - incorrect / 4).toFixed(2));
  ...
  const handleAddNewBook = (e) => {
    ...
    const questionsD = Math.max(1, parseInt(newBookData.questionsPerDeneme) || 18);
    ...
    const newBook = {
      ...
      color: newBookData.subject === "Coğrafya" ? "emerald" : newBookData.subject === "Tarih" ? "amber" : "purple",
              ...
              >
                ...
                <div className="p-5 md:p-6 bg-slate-500/5">
                  <div className="text-[11px] font-black uppercase tracking-wider text-slate-400 mb-3 flex items-center justify-between">
                    <span>Branş Denemeleri Matrix</span>
                      ...
                      <table className="w-full text-left text-xs border-collapse">
                        <thead>
                          <tr className={`border-b text-[10px] uppercase font-black tracking-wider ${isDark ? "border-slate-800 text-slate-400" : "border-slate-200 text-slate-500"}`}>
                            ...
                            <th className="py-3 px-3">Doğru</th>
                            <th className="py-3 px-3">Yanlış</th>
                            <th className="py-3 px-3">Boş</th>
                        ...
                        <tbody className="divide-y divide-slate-100 dark:divide-slate-800/60">
                          {book.denemes.map(d => (
                            <tr key={d.id} className={`hover:bg-slate-500/5 transition ${d.solved ? (isDark ? "bg-emerald-500/5" : "bg-emerald-50/50") : ""}`}>
                              ...
                              <td className="py-2.5 px-3 font-bold text-emerald-400">{d.solved ? d.correct : "-"}</td>
                              <td className="py-2.5 px-3 font-bold text-rose-400">{d.solved ? d.incorrect : "-"}</td>
                              <td className="py-2.5 px-3 font-medium text-slate-400">{d.solved ? d.empty : "-"}</td>
              ...
              return (
                <form onSubmit={handleSaveDeneme} className="space-y-4">
                  <div>
                    ...
                    <p className="text-xs text-slate-400">Bu denemedeki soru sayısı: {maxQ}</p>
                  </div>
                  ...
                  <div className="p-4 rounded-xl bg-gradient-to-r from-purple-500/10 to-indigo-500/10 border border-purple-500/20 text-center">
                    <div className="text-[10px] font-black uppercase tracking-wider text-purple-400">Hesaplanan Net</div>
                  ...
                  {/* Score Inputs (Correct, Incorrect, Empty) */}
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input
                    ...
                    min="1"
                    max="150"
                    value={newBookData.questionsPerDeneme}
